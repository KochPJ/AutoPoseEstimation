...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num_pt, add_noise, noise_trans, refine, data_set_name):
        if mode == 'train':
    ...
    def __getitem__(self, index):
        ...
        label = np.array(Image.open('{0}/{1}.gen.label.png'.format(self.label_root, self.list[index])))
...
#            img_masked = img
        ...
        target_r = np.array(meta['rotation']).reshape((3, 3))
        target_t = np.array(meta['position'])
